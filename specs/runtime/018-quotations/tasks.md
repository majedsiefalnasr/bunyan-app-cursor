...
- [ ] T001 Define RFQ/Quotation enums (statuses) at `backend/app/Enums/` (or consistent existing location)
- [ ] T002 Create migrations for `rfqs`, `rfq_items`, `quotations`, `quotation_items` in `backend/database/migrations/` (with FK + indexes + down())
...
- [ ] T006 Add policies `RfqPolicy`, `QuotationPolicy` under `backend/app/Policies/` (customer ownership + contractor supplier_profile gating)
...
- [ ] T010 Implement `QuotationService` under `backend/app/Services/` (submit/revise rules, accept award via `DB::transaction`, compare builder)

- [ ] T011 Implement `RfqController` under `backend/app/Http/Controllers/Api/V1/` (index/store/show/send/compare)
- [ ] T012 Implement `RfqQuotationController` under `backend/app/Http/Controllers/Api/V1/` (index/store/accept)
- [ ] T013 Register routes in `backend/routes/api.php` under `auth:sanctum` + appropriate `role:` middleware
...
- [ ] T016 Add feature tests `backend/tests/Feature/Api/V1/RfqApiTest.php` (RBAC matrix + validations + pagination)
- [ ] T017 Add feature tests `backend/tests/Feature/Api/V1/QuotationApiTest.php` (deadline enforcement + own-quote access + award flow)

- [ ] T018 Add new Nuxt pages for customer RFQs: `frontend/pages/rfqs/index.vue`, `frontend/pages/rfqs/new.vue`
- [ ] T019 Add customer RFQ details + compare pages: `frontend/pages/rfqs/[id]/index.vue`, `frontend/pages/rfqs/[id]/compare.vue`
...
- [ ] T025 Run full validation gate commands and capture any fixes (lint/typecheck/tests/migrations)
